          ...
          <div className="flex flex-col gap-y-3 pt-3">
            {/* {data?.industry !== "" && (
              <div className="w-full mt-2">
                ...
                <p className="text-black-3 text-[12px] leading-[20px]">
                  {" "}
                ...
                <>
                  {data?.desiredJobTitle?.map((value, index) => {
                    return (
                      <span
                        key={index * 8}
                        className="text-black-3 text-[12px] leading-[20px] font-medium"
                      >
                        {value}
                        {index !== data?.desiredJobTitle?.length - 1 && ", "}
                      </span>
                    );
                  })}
                </>
            ...
            {data?.desiredSalary !== "" && (
              <div className="w-full mt-2">
                ...
                <p className="text-black-3 text-[12px] leading-[20px]">
                  {" "}
                  {data?.desiredSalary ? data?.desiredSalary + " USD" : "-"}
                </p>
              </div>
            )}
                ...
                <>
                  {data?.skills.map((value, index) => {
                    return (
                      <span
                        key={index * 9}
                        className="text-black-3 text-[12px] leading-[20px]"
                      >
                        {value}
              ...
              {data?.relocation?.anywhere === true ? (
                ...
                <>
                  {data?.relocation?.onlyNearMe?.locations.length !== 0 ? (
                    <>
                      {data?.relocation?.onlyNearMe?.locations?.map(
                        (value, index) => {
                          return (
                            <span
                              key={index * 9}
                            ...
                            >
                              {value}
                              {index !==
                                data?.relocation?.onlyNearMe?.locations
                                  ?.length -
                                  1 && ", "}
                            </span>
                          );
