...
const { jobStatusOptions } = dropdownOptions;
...
function JobDetails({
  ...
  const firstLetter = extractedData?.employer?.title
    ? extractedData?.employer?.title.trim().charAt(0).toUpperCase()
    : "";
    ...
    <Core.Card className={`border p-0`} p={"0"}>
      ...
      >
        <div className="flex justify-end gap-x-4">
          ...
          {pageType === "view" && (
            <span className="text-gray-6 text-[16px] leading-[20px] capitalize font-semibold pl-1 py-1">
              Job Status:
              <span className="text-purple-4"> {extractedData?.jobStatus}</span>
            </span>
          )}
          {pageType === "edit" && (
        ...
        <div className="flex justify-between -mt-3">
          <div className="relative w-[100px]">
            <div
              className={`absolute ${
                !pageType === "quickView" && "-top-3"
              } flex justify-center items-center w-[80px] h-[80px] bg-gray-7 overflow-hidden`}
            >
              <Avatar
                size={80}
          ...
          <div className="w-full h-full flex justify-between pl-2">
            <div>
              ...
              <p className="text-gray-6 text-[12px] leading-[20px]">
                <span>
                  {extractedData?.employer?.jobMode}
                  {extractedData?.jobType && (
                    <span>{extractedData?.jobType},&nbsp;&nbsp;</span>
                  )}
                </span>
                {extractedData?.employer?.address}
              </p>
              <div className="flex justify-start gap-x-6 text-gray-14 text-[12px] leading-[20px] py-1">
                <span className="flex justify-start items-center gap-x-1">
                  <Icon name="Calender2" size={18} />{" "}
                  {extractedData?.postedDate}
                </span>{" "}
                <span className="flex justify-start items-center gap-x-1">
                  <Icon name="Currency" />
                  {extractedData?.salary?.value}
                </span>
              </div>
              ...
              <div className="flex justify-start gap-x-2 pt-3">
                <h6 className="text-black-2 text-[12px] leading-[20px]">
                  Share this job:
                </h6>
                <div className="flex justify-start items-center gap-x-6">
                  <Tooltip title="Coming soon: Share this job on social media!">
      ...
      >
          ...
          <div className="pb-8">
            ...
            <ul className="flex justify-start flex-wrap gap-x-3 text-gray-6 list-disc pl-4 mt-4">
              ...
              {extractedData.benefits.map((benefit) => (
                <li
                  className={`${
                    pageType === "quickView" ? "w-[32%]" : "w-[18%]"
                  } text-[14px] leading-[18px] mb-3`}
                >
                  {benefit}
                </li>
              ))}
        ...
        <div className="responsibilities-container pb-4">
          ...
          <div
            dangerouslySetInnerHTML={{
              __html: extractedData?.responsibilities || "-",
            }}
          ></div>
        </div>
        ...
        <div className="responsibilities-container pb-4">
          ...
          <div
            dangerouslySetInnerHTML={{
              __html: extractedData?.qualification || "-",
            }}
          ></div>
        </div>
        ...
        <div className="responsibilities-container pb-4">
          ...
          <div
            dangerouslySetInnerHTML={{ __html: extractedData?.skills || "-" }}
          ></div>
        </div>
        ...
        {pageType === "edit" && (
          <div className="flex justify-start gap-x-3 mt-8">
            <Core.Button onClick={handleNext} type="narrow">
              Save Chagnes
            </Core.Button>
            <Core.Button
              // onClick={handleBack}
